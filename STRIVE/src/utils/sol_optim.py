...
def run_find_solution_optim(
    ...
    other_match_traj = other_match_traj.view(
        other_match_traj.size(0), 1, other_match_traj.size(1), 4
    )
    ...
    for oidx in pbar_optim:
        ...
        def closure():
            ...
            tgt_loss_input_z = collate_tgt_other_z(
                scene_graph, tgt_z, other_z_all.detach()
            )
            ...
            match_loss_input_z = collate_tgt_other_z(
                scene_graph, tgt_z.detach(), other_z_all
            )
            match_decoder_out = model.decode_embedding(
                match_loss_input_z, embed_info, scene_graph, map_idx, map_env
            )

            # compute loss for target agent
            tgt_future_pred = model.get_normalizer().unnormalize(
                tgt_decoder_out["future_pred"]
            )  # (NA, 1, T, 4)
            tgt_future_pred = tgt_future_pred.transpose(0, 1).reshape(NA, future_len, 4)
            tgt_avoid_loss_dict = avoid_loss(tgt_future_pred, tgt_z, tgt_prior_distrib)
            loss_dict = {"tgt_" + k: v for k, v in tgt_avoid_loss_dict.items()}

            # compute loss for other agents
            other_future_pred = model.get_normalizer().unnormalize(
                match_decoder_out["future_pred"]
            )
            other_future_pred = other_future_pred[~tgt_mask]
            match_loss_dict = match_loss(
                other_future_pred, other_match_traj, other_z_all, other_prior_distrib
            )
    ...
    with torch.no_grad():
        sol_decoder_out = model.decode_embedding(
...
def compute_sol_success(
    final_result_traj, model, scene_graph, map_env, map_idx, use_map_coll=True
):
    """
    ...
    planner_coll_all, planner_coll_time = check_single_veh_coll(
        sol_fut, planner_lw, other_fut, other_lw
    )
